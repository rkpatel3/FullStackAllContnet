...
// team = all of the possible players
var team = [];
// currentTeam = the ones currently playing
var currentTeam = [];
// only the starters
var starters = [];
// only the subs
...
function initPlayers() {
  ...
  if (starters.length + subs.length < 3) {
    ...
    // Every game should begin with the starters as the current team
    // Subs may occur later in any round
    currentTeam = starters;
...
function promptPlayerCreation() {
  ...
  inquirer
    ...
    .then(function(answers) {
      ...
      // team should contain all players
      team.push(player);
...
function playRound() {
  ...
  for (var i = 0; i < currentTeam.length; i++) {
    teamOffense += currentTeam[i].offense;
    teamDefense += currentTeam[i].defense;
  }
          ...
          .then(function(subIn) {
            ...
            inquirer
              ...
              .then(function(subOut) {
                ...
                for (var i = 0; i < currentTeam.length; i++) {
                  if (currentTeam[i].name === subOut.sub) {
                    subs[number] = currentTeam[i];
                    currentTeam[i] = sideline;
